...
There is a dramatic change, due to the fact that there were very few central families, and some of the most prominent had self-loops. We can see the graph with duplicated nodes here

```{r graph}
dup.graph <- dup.nodes.from.data.frame(
  data.frame(married.doges$Family.doge, married.doges$Family.dogaressa)
  )
plot(dup.graph)
```
